...
class ProposeScheduledCarpoolingRepository(ProposeScheduledCarpoolingRepositoryInterface):
    POSTGRES_TABLE_NAME: str = "propose_scheduled_carpooling"

    def insert(self,
               data: PassengerScheduledCarpoolingTable) -> int:
        query = f"""
                INSERT INTO carmate.{self.POSTGRES_TABLE_NAME} (label, starting_point, destination, start_hour, start_date, end_date, days, passenger_id)
    ...
    def has_same_time_proposed_scheduled_carpooling(self,
        ...
        query = f"""
                SELECT EXISTS(
                    SELECT 1
                    FROM carmate.{self.POSTGRES_TABLE_NAME} AS sc
    ...
    def get_user_id_from_scheduled_carpooling(self, propose_scheduled_carpooling_id: int) -> int:
        query = f"""
                SELECT pp.id
                FROM carmate.{self.POSTGRES_TABLE_NAME} pc
                INNER JOIN carmate.{PassengerProfileRepository.POSTGRES_TABLE_NAME} pp ON (pc.passenger_id = pp.id)
                WHERE pc.id = %s
        """
        user_id: tuple
        with establishing_connection() as conn:
        ...
        query = f"""
                    WITH ProposedSchedule AS (
                        SELECT
                            ...
                            pp.user_id
                        FROM
                            carmate.{self.POSTGRES_TABLE_NAME} pc
                            INNER JOIN carmate.{PassengerProfileRepository.POSTGRES_TABLE_NAME} pp ON (pc.passenger_id = pp.id)
                        WHERE
                            pc.id = %s
                    ),
                    ...
                    AlreadyReservedDates AS (
                        ...
                        FROM
                            carmate.{BookingCarpoolingRepository.POSTGRES_TABLE_NAME} rc
                            INNER JOIN carmate.{CarpoolingRepository.POSTGRES_TABLE_NAME} c ON (rc.carpooling_id = c.id)
                            INNER JOIN ReservationDates rd ON (rd.user_id = rc.user_id)
                        WHERE
                            rd.reserved_date = c.departure_date_time::date
                    ...
                    ReservationDatesFiltered AS (
                        ...
                        FROM
                            ReservationDates
                        WHERE
                            reserved_date NOT IN (SELECT departure_date_time FROM AlreadyReservedDates)
                    ),
                    NumberOfReservations AS (
                        SELECT rc.carpooling_id, COUNT(*) AS reserved
                    ...
                    FROM
                        carmate.{CarpoolingRepository.POSTGRES_TABLE_NAME} c
                    INNER JOIN ReservationDatesFiltered rd ON c.starting_point = rd.starting_point
                    ...
                    LEFT JOIN NumberOfReservations nr ON c.id = nr.carpooling_id
        ...
        query = f"""
                    WITH ProposedSchedule AS (
                        ...
                        FROM
                            ...
                            INNER JOIN carmate.{PassengerProfileRepository.POSTGRES_TABLE_NAME} pp ON (pc.passenger_id = pp.id)
                        WHERE
                            pc.starting_point = %s::double precision[] 
